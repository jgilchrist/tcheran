...
fn play_game(rand: &mut impl Rng, config: &DatagenConfig) -> GameResult {
    ...
    let result = match (game.player, result) {
        (_, WDL::Draw) => WDL::Draw,
        (Player::White, WDL::Win) => WDL::Win,
        (Player::White, WDL::Loss) => WDL::Loss,
        // Convert wins/losses as black to be in white's perspective.
        (Player::Black, WDL::Win) => WDL::Loss,
        (Player::Black, WDL::Loss) => WDL::Win,
    };
...
fn get_config_from_args(args: &Cli) -> DatagenConfig {
    ...
    let tb = args.syzygy_path.as_ref().map(load_tablebases);
...
fn load_tablebases(syzygy_path: &PathBuf) -> Tablebase {
