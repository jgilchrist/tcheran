...
8/1p6/p5R1/k7/Prpp4/K7/1NP5/8 w - - bm Rb6 Rg5+; id "WAC.274";
...
fn parse_spec(line: &str) -> Result<WinAtChessSpec> {
    let (rest, id) = line.split_once("; id ").unwrap();
    ...
    let (fen, best_moves) = rest.split_once(" bm ").unwrap();
    let mut fen = fen.to_string();
    fen.push_str(" 0 1");

    let game = Game::from_fen(&fen).unwrap();

    let moves = best_moves
        .split_whitespace()
        .map(|m| chess::san::parse_move(&game, m).unwrap())
        .collect();

    Ok(WinAtChessSpec { id, game, moves })
}

fn run_spec(spec: &WinAtChessSpec, time_control: &TimeControl) -> Result<()> {
    print!("{}: ", spec.id);
    stdout().lock().flush()?;
    ...
    if spec.moves.iter().any(|m| *m == best_move) {
        print!(
            "Best move! {:?}",
            chess::san::format_move(&spec.game, &best_move)
        );
    } else {
        print!(
            "Bad move! Got {:?} but wanted {:?}",
            chess::san::format_move(&spec.game, &best_move),
            spec.moves
                .iter()
                .map(|m| chess::san::format_move(&spec.game, m))
                .collect::<Vec<_>>(),
        );
    }
...
fn main() -> Result<()> {
    ...
    let specs: Vec<WinAtChessSpec> = WIN_AT_CHESS_EPD
        .lines()
        .map(|l| parse_spec(l).unwrap())
        .collect();
    ...
    let time_control = TimeControl::ExactTime(Duration::from_secs(10));

    for spec in specs {
        run_spec(&spec, &time_control)?
    }
